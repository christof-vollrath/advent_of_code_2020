...
class Day20_Part1 : FunSpec({
    context("tile operations") {
        ...
        context("all variations") {
            ...
            ...
            """.trimIndent())
            val allVariants = tile.allVariations()
            allVariants.forEach { println(it.toPrintableString() + "\n---\n")}
...
fun List<List<Char>>.allVariations() = sequence {
    val original = this@allVariations
    yield(original)
    yield(original.flipX())
    yield(original.flipY())
    val turned1 = original.turnRight()
    println("turned1\n" + turned1.toPrintableString() + "\n---\n")
    yield(turned1)
    val turned2 = turned1.turnRight()
    println("turned2\n" + turned2.toPrintableString() + "\n---\n")
    yield(turned2)
    val turned3 = turned2.turnRight()
    println("turned3\n" + turned3.toPrintableString() + "\n---\n")
    yield(turned3)
}.toSet()

fun List<List<Char>>.flipX() = reversed()
fun List<List<Char>>.flipY() = map { it.reversed() }
